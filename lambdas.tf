...
module "lambdas" {
  ...
  allowed_triggers = contains(keys(each.value), "cw_event_rule_id") ? length(each.value.cw_event_rule_id) > 0 ? merge(local.common_lambda_permissions, {
  ...
  } }) : local.common_lambda_permissions : local.common_lambda_permissions
