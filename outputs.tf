  ...
  value = tomap({
    api_gateway = {
      for k, v in var.api_gateway.domains : k => {
        ...
        hosted_zone_id     = try(aws_apigatewayv2_domain_name[k].domain_name_configuration[0].hosted_zone_id, null)
        target_domain_name = try(aws_apigatewayv2_domain_name[k].domain_name_configuration[0].target_domain_name, null)
      }
    }
    admin_portal = {
