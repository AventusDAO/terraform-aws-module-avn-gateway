...
module "amazonmq" {
  ...
  mq_admin_user     = [jsondecode(data.aws_secretsmanager_secret_version.gateway_amazonmq.secret_string)["admin_user"]]
  mq_admin_password = [jsondecode(data.aws_secretsmanager_secret_version.gateway_amazonmq.secret_string)["admin_passw"]]
  # Lambda is connecting to amazonmq by fetching username and password keys
  mq_application_user     = [jsondecode(data.aws_secretsmanager_secret_version.gateway_amazonmq.secret_string)["username"]]
  mq_application_password = [jsondecode(data.aws_secretsmanager_secret_version.gateway_amazonmq.secret_string)["password"]]
}